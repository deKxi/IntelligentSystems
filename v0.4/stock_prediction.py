...
PREDICT_DAYS_AHEAD = 7 # number of days ahead to base our prediction | Default: 1
TEST_SIZE = 0.2
...
RUN_NAME = "Multivariate_Multistep_LSTM"
...
if RUN_MODEL:
    ...
    if SAVE_LOSS_PLOT or DISPLAY_LOSS:
        ...
        if SAVE_LOSS_PLOT: plt.savefig(f'./logs/{RUN_NAME}/{run_name}/loss_plot.jpg', dpi=300, bbox_inches='tight')
        # Display the plot
    ...
    # (Number of predictions to make, history/days to predict from, Number of features)
    ...
    predicted_prices['price_error'] = predicted_prices[PRICE_VALUE] - actual_close_prices
...
if COMPARE_RUNS:
    ...
    plt.figure(figsize=(12, 6))
    fig = plt.get_current_fig_manager()
    fig.set_window_title(f'Model: {COMPANY} Loss Comparison')
    style_cur = col_cur = 0
    style_list = ['-', '--', '-.', ':']
    col_list = ['r', 'g', 'b', 'k']
